#!/usr/bin/env python
# Created by "Thieu" at 14:18, 21/03/2023 ----------%                                                                               
...
from mealpy import Problem, Termination
...
class Multitask:
    ...
    """
    def __init__(self, algorithms=(), problems=(), terminations=None, **kwargs):
        ...
        if terminations is not None:
            self.terminations = self.validator.check_list_tuple("terminations", terminations, "Termination")
    ...
    def __run__(self, id_trial, model, problem, termination=None):
        _, best_fitness = model.solve(problem, termination=termination)
        return {
            "id_trial": id_trial,
        ...
        for id_model, model in enumerate(self.algorithms):
            ...
            for id_prob, problem in enumerate(self.problems):
                ...
                if self.terminations is not None:
                    term = self.terminations[id_prob]
                ...
                if mode == "parallel":
                    with parallel.ProcessPoolExecutor(n_workers) as executor:
                        list_results = executor.map(partial(self.__run__, model=model, problem=problem, termination=term), trial_list)
                    ...
                    for idx in trial_list:
                        result = self.__run__(idx, model, problem, termination=term)
                        convergence_trials[f"trial_{result['id_trial']}"] = result['convergence']
                        best_fit_trials.append(result['best_fitness'])
                        if verbose:
                            print(f"Solving problem: {problem.get_name()} using algorithm: {model.get_name()}, on the: {result['id_trial']} trial")
